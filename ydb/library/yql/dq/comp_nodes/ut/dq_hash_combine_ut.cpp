...
// Spiller whose writes never complete, to park the spilling coroutine on a pending future
...
Y_UNIT_TEST_SUITE(TDqHashCombineTest) {
    ...
    Y_UNIT_TEST_QUAD(TestTeardownDuringStateSpilling, UseLLVM, UseFlow) {
        ...
        // teardown mid-spill must not double-release arena values (#40326, run under ASAN)
        RunDqAggregateEarlyStopTest(
            setup,
            UseFlow,
            streamCreator,
