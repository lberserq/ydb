...
// Minimal cookie allocator so TExecutorGCLogic can be constructed directly.
// Only Cookies->Gen is read on the paths exercised here; GroupBy() is never called,
// so the slot group value is a placeholder.
TAutoPtr<NPageCollection::TSteppedCookieAllocator> MakeGCCookies(
        const TTabletStorageInfo& info, ui32 generation) {
