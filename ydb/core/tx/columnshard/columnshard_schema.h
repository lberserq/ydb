...
struct Schema: NIceDb::Schema {
    ...
    struct InsertTable: NIceDb::Schema::Table<InsertTableId> {
        ...
        using TColumns = TableColumns<Committed, PlanStep, WriteTxId, PathId, DedupId, BlobId, Meta, IndexPlanStep, IndexTxId, SchemaVersion,
            BlobRangeOffset, BlobRangeSize, InsertWriteId>;
    };
    ...
    struct IndexPortions: NIceDb::Schema::Table<PortionsTableId> {
        ...
        using TColumns = TableColumns<PathId, PortionId, SchemaVersion, XPlanStep, XTxId, Metadata, ShardingVersion, MinSnapshotPlanStep,
            MinSnapshotTxId, CommitPlanStep, CommitTxId, InsertWriteId>;
    };
    ...
    using TTables = SchemaTables<Value, TxInfo, SchemaPresetInfo, SchemaPresetVersionInfo, TtlSettingsPresetInfo, TtlSettingsPresetVersionInfo,
        ...
        MoveDataRows, CutHistoryRequests>;
    ...
    static void AddNormalizerEvent(NIceDb::TNiceDb& db, const TString& normalizerId, const TString& eventType, const TString& description) {
        db.Table<NormalizerEvents>()
            ...
            .Update(NIceDb::TUpdate<NormalizerEvents::Instant>(TInstant::Now().MicroSeconds()),
                NIceDb::TUpdate<NormalizerEvents::EventType>(eventType), NIceDb::TUpdate<NormalizerEvents::Description>(description));
    }
    ...
    static void UpdateTxInfoSource(NIceDb::TNiceDb& db, ui64 txId, const TActorId& source, ui64 cookie) {
        db.Table<TxInfo>().Key(txId).Update(NIceDb::TUpdate<TxInfo::Source>(source), NIceDb::TUpdate<TxInfo::Cookie>(cookie));
    }
    ...
    static void SaveSchemaPresetVersionInfo(
        NIceDb::TNiceDb& db, ui64 id, const NOlap::TSnapshot& version, const NKikimrTxColumnShard::TSchemaPresetVersionInfo& info) {
        TString serialized;
    ...
    static void SaveSchemaPresetDropVersion(NIceDb::TNiceDb& db, ui64 id, const NOlap::TSnapshot& dropVersion) {
        db.Table<SchemaPresetInfo>().Key(id).Update(NIceDb::TUpdate<SchemaPresetInfo::DropStep>(dropVersion.GetPlanStep()),
    ...
    static void SaveTableSchemeShardLocalPathId(
        NIceDb::TNiceDb& db, const TInternalPathId pathId, const TSchemeShardLocalPathId schemeShardLocalPathId) {
        db.Table<TableInfo>()
            .Key(pathId.GetRawValue())
            .Update(NIceDb::TUpdate<TableInfo::SchemeShardLocalPathId>(schemeShardLocalPathId.GetRawValue()));
    }
    ...
    static void CopySchemeShardLocalPathIdV1(NIceDb::TNiceDb& db, const TInternalPathId pathId,
        ...
        if (dropVersion) {
            db.Table<TableInfoV1>()
                ...
                .Update(NIceDb::TUpdate<TableInfoV1::DropStep>(dropVersion->GetPlanStep()),
        ...
        db.Table<TableInfoV1>()
            .Key(pathId.GetRawValue(), dstSchemeShardLocalPathId.GetRawValue())
            .Update(NIceDb::TUpdate<TableInfoV1::IsReadOnly>(isReadOnly));
    }

    static void SaveTableVersionInfo(
        NIceDb::TNiceDb& db, TInternalPathId pathId, const NOlap::TSnapshot& version, const NKikimrTxColumnShard::TTableVersionInfo& info) {
        TString serialized;
        Y_ABORT_UNLESS(info.SerializeToString(&serialized));
        db.Table<TableVersionInfo>()
            .Key(pathId.GetRawValue(), version.GetPlanStep(), version.GetTxId())
            .Update(NIceDb::TUpdate<TableVersionInfo::InfoProto>(serialized));
    }

    static void SaveTableDropVersion(NIceDb::TNiceDb& db, TInternalPathId pathId, ui64 dropStep, ui64 dropTxId) {
        db.Table<TableInfo>().Key(pathId.GetRawValue()).Update(NIceDb::TUpdate<TableInfo::DropStep>(dropStep),
    ...
    static void SaveLongTxWrite(NIceDb::TNiceDb& db, const TInsertWriteId writeId, const ui32 writePartId,
        const NLongTxService::TLongTxId& longTxId, const std::optional<ui32> granuleShardingVersion) {
        NKikimrLongTxService::TLongTxId proto;
        longTxId.ToProto(&proto);
        TString serialized;
        Y_ABORT_UNLESS(proto.SerializeToString(&serialized));
        db.Table<LongTxWrites>().Key((ui64)writeId).Update(NIceDb::TUpdate<LongTxWrites::LongTxId>(serialized),
